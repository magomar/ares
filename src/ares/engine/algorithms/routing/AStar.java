...
public class AStar extends AbstractPathFinder {
        ...
        while(!openSet.isEmpty()){
            ...
            for(Map.Entry<Direction, TileModel> iter : current.getTile().getNeighbors().entrySet()){
                ...
                if(neighbour == null){
                    ...
                    if(tentativeG < Integer.MAX_VALUE) openSet.add(neighbour);
                }
    ...
    @Override
    public EnumSet<Direction> getDirections(TileModel o, TileModel d) {
        throw new UnsupportedOperationException("Not supported yet.");
    }

    private Path createPath(Node current) {
        Path path = new Path();
        return path;
    }
