...
//    private static final int[][] NEIGHBORS_DJ = { {-1, -1, 0, 1, 0, -1}, {-1, 0, 1, 1, 1, 0}};
    ...
    public HexGridCell(int radius) {
        RADIUS = radius;
        WIDTH = radius * 2;
        HEIGHT = (int) (((double) radius) * Math.sqrt(3));
        SIDE = radius * 3 / 2;
...
//        int ty = y - (ci % 2) * HEIGHT / 2;
        int ty = y - ((ci + 1) % 2) * HEIGHT / 2;
        int cj = (int) Math.floor((double) ty / (double) HEIGHT);
        int cy = ty - HEIGHT * cj;
...
//            setCellIndex(ci - 1, cj + (ci % 2) - ((cy < HEIGHT / 2) ? 1 : 0));
            setCellIndex(ci - 1, cj + ((ci + 1) % 2) - ((cy < HEIGHT / 2) ? 1 : 0));
        }
    }
}
